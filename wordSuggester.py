...
def evaluteList(attempt, clue, wordies):
    is_in_word = []
    is_in_position = {}
    wrong_position = {}
    not_in_word = []
    ...
    for posi, num in enumerate(clue):
        match num:
            case "3":
                if attempt[posi] not in is_in_word:
                    is_in_word.append(attempt[posi])
                is_in_position[posi] = attempt[posi]
                if attempt[posi] in not_in_word:
                    not_in_word.remove(attempt[posi])
    for posi, num in enumerate(clue):
        match num:
            case "2":
                if attempt[posi] not in is_in_word:
                ...
                if attempt[posi] in not_in_word:
                    not_in_word.remove(attempt[posi])
                wrong_position[posi] = attempt[posi]
    for posi, num in enumerate(clue):
        match num:
            case "1":
                if attempt[posi] in is_in_word:
                    wrong_position[posi] = attempt[posi]
                    if attempt[posi] not in letter_count:
                        letter_count[attempt[posi]] = 2
                    else:
                        letter_count[attempt[posi]] += 1
                elif attempt[posi] not in not_in_word:
                    not_in_word.append(attempt[posi])


    for word in wordies:
        if clue == "11111":
            curr_wordies.append(word)
        elif attempt == word and clue != "33333":
            if word in curr_wordies:
                curr_wordies.remove(word)
        else:
